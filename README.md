...
Все обязательные и дополнительные пункты.
...
# Отправка email-сообщений с квотами

Осуществляется через celery-задачу.
...
![image](https://github.com/UncleVasya/sibdev_test_2/assets/1286157/806bce15-1494-4215-aa27-d7f3be3454c4)


По умолчанию отправка сообщений настроена с ящика `oleg-sibdev-test@yandex.ru`. 
Настройки почтового ящика можно посмотреть в `env.dev`. 
...
Функционал, который покрыт тестами:
```
- загрузка данных из внешнего сервиса:
---- загрузка истории цен;
---- загрузка дневных котировок;
- работа api для добавления отслеживаемой валюты:
---- необходимость авторизации;
---- успешное добавление отслеживаемой валюты;
---- невозможность повторного добавления, если валюта уже отслеживается данным пользователем;
- работа api для получения дневных котировок:
---- получение котировок пользователем, у которого имеются отслеживаемые валюты;
---- получение котировок пользователем, который не добавлял отслеживаемые валюты;
---- получение котировок анонимным пользователем;
---- сортировка по полю value;
- работа api аналитики:
---- необходимость авторизации;
---- вычисление дополнительных аналитических данных по заданию;
---- фильтрация по интервалу дат;
```

Функционал, который еще можно покрыть тестами:
```
- соответствие указанной схеме;
- работа celery-задач;
